...
top_left_x = (s_width - (block_size*cols)) // 2
top_left_y = s_height - (block_size*rows)
...
class Piece(object):  #TODO: cambiar nombre
  ...
  def __init__(self, cols, rows, shape):
    self.x = cols #TODO: width
    self.y = rows #TODO: height
    self.shape = shape
...
def create_grid(locked_positions={}):
    grid = [[(0,0,0) for x in range(cols)] for x in range(rows)]
